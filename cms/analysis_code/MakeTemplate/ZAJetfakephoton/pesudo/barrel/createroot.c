...
void histo(){
 ...
 Double_t xhigh= 0.02;
  for(Int_t i=0;i<7;i++){
      h_sieie[i]=new TH1D(Form("hsieie_pt%0.f-%0.f",lowpt[i],highpt[i]),"chiso<0.441 and sieie<0.01022",bin,0,xhigh);//Zjets && leding_photon && medium_cut && photon_chiso<1.416
      hfake[i]=new TH1D(Form("hfake_pt%0.f-%0.f",lowpt[i],highpt[i]),"chiso<0.441 isprompt!=1",bin,0,xhigh);//Zjets && leding_photon && medium_cut && photon_chiso<1.416
      htrue[i]=new TH1D(Form("htrue_pt%0.f-%0.f",lowpt[i],highpt[i]),"chiso<0.441 isprompt=1",bin,0,xhigh);//Zjets && leding_photon && medium_cut && photon_chiso<1.416
...
f1->Close();
TFile *f2 = new TFile("../outZA_realphoton.root");     
