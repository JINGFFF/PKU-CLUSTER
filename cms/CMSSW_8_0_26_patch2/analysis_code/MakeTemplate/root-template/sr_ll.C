...
int main(){
	gStyle->SetOptStat(0);
        MvaFlag=0;
	THStack* hs = new THStack("hs","massVlep");
...
//			LEP = l1_weight==1 && HLT_Mu2 > 0 && lep==13 && ptlep1 >20. && ptlep2 >20.&& abs(etalep1) <2.4 &&abs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3;/*cout<<name<<endl;*/
		...
		if(MvaFlag==false) {h->Fill(massVlep/*,scalef*pileupWeight*lumiWeight*35.86*0.96*/);m++;}
      ...
      for(Int_t j=0;j<6;j++){
		...
		if(MvaFlag==false)
		{
			if(name.Contains("ZJets") && photon_isprompt[position]==-1/*&&massVlep<100 && massVlep>80 && Mva >101 && photon_drla[position]>1 && photon_drla2[position]>1 */)	
				{hh->Fill(massVlep,scalef*pileupWeight*lumiWeight*35.86*0.96);m++;}
...
void draw(THStack* hs, TH1D* h1, TH1D* h2, TH1D* h3){
	...
	bottom_pad->SetTopMargin(0);
        ...
        gr->SetTitle("");
        gr->GetXaxis()->SetTitle( "massVlep [GeV] ");
	...
	if(MvaFlag==true) {c1->Print(Save+"Mva.eps");c1->Print(Save+"Mva.C");}
	...
	h3->SetFillColor(0);
        ...
        h3->Draw("same hist");
	t2->Draw();
	can->Print(Save+"massVlep_zaAndzj.eps");
}
