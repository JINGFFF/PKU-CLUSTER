...
using namespace std;
	...
	Double_t photonet, photoneta, photonphi, photone;
	...
	Double_t ptlep1,ptlep2,etalep1,etalep2,l1_weight,philep1,philep2;
	...
	Double_t yVlep,ptVlep,phiVlep;
        Int_t bin1=45,bin2=45;
        Double_t low1=30,low2=50;
        Double_t high1=200,high2=400;

	TString dir = "../WAJetfakephoton/drawSR/";
//	TString dir = "/data/pku/home/anying/cms/file_in_cms/files_weighted/";
//	TString dir = "/data/pku/home/anying/cms/CMSSW_8_0_26_patch2/analysis_code/selection_code/";
...
TH1D* filedata(TString name,Bool_t MvaFlag){
	...
	TH1D* h;
	if(MvaFlag==false) h = new TH1D("h","massVlep",bin1,low1,high1);
	...
	for(Int_t i=0;i<nentries;i++){
      ...
      for(Int_t j=0;j<6;j++){
		...
		if(i%10000000==0) cout<<i<<" "<<nentries<<endl;
			LEP = l1_weight==1  && lep==13 && ptlep1 >20. && ptlep2 >20.&& abs(etalep1) <2.4 &&abs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && ((run_period<7 && HLT_Mu5 > 0)|| (run_period==7 && HLT_Mu2 > 0))/*&& massVlep>20 && massVlep<200*/; /*cout<<name<<endl;*/
		for(Int_t j=0;j<6;j++)
		{
		...
		if(MvaFlag==false) {h->Fill(massVlep);m++;}
...
TH1D* filemc(TString name,Bool_t MvaFlag){
	Double_t m=0,k=0,n=0;
        TH1D* hh;
	if(MvaFlag==false)  hh = new TH1D("hh","massVlep",bin1,low1,high1);
...
//        nentries=10000;
	...
	for(Int_t i=0;i<nentries;i++){
      ...
      for(Int_t j=0;j<6;j++){
		...
		if(MvaFlag==true)
		{
                        if(name.Contains("ZJets")&& photon_isprompt[position]==-1)
                                hh->Fill(Mva,scalef*pileupWeight*lumiWeight*35.86*0.96);
                        if(name.Contains("ZA")&& photon_isprompt[position] == 1/*&&massVlep>80*/)
                                {hh->Fill(Mva,scalef*pileupWeight*lumiWeight*35.86*0.96);m++;}
...
void draw(THStack* hs, TH1D* h1, TH1D* h2, TH1D* h3){
	...
	h1->SetMarkerStyle(20);
        ...
        cout<<"OK3"<<endl;
	c1->cd();
	TLine* line1 = new TLine(low2,1.,high2,1);
	...
	bottom_pad->SetTopMargin(0);
        ...
        gr->SetTitle("");
        gr->GetXaxis()->SetTitle( "Z+#gamma mass [GeV] ");
        gr->GetYaxis()->SetTitle("Data / MC ");
        gr->GetXaxis()->SetRangeUser(low2,high2);
        ...
        if(MvaFlag==false){c1->Draw();c1->Print(Save+"massVlep.eps");c1->Print(Save+"massVlep.C");top_pad->SetLogy();c1->Print(Save+"massVlep_log.eps");c1->Print(Save+"massVlep_log.C");}
	if(MvaFlag==true) {c1->Print(Save+"Mva.eps");c1->Print(Save+"Mva.C");top_pad->SetLogy();c1->Print(Save+"Mva_log.eps");c1->Print(Save+"Mva_log.C");}
	...
	h3->SetFillColor(0);
        ...
        h3->Draw("same hist");
	t2->Draw();
	can->Print(Save+"Mva_zaAndzj.eps");
}
